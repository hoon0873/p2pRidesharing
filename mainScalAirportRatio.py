    ...
    for i in range(D):
        ...
        if randPart <= dp:
            if randArrive >= 0.5:
                ...
                des = dWork
                time = flightTime
                delta = int(rd.uniform(0,0.3)*PRECISION)
...
timeF = open("data\ScalabilityAirportRatio.txt", 'a+')
timeF.seek(0)
newRho = 1.2
RATIOPRECISION = 10
TotalAgents = 100
for RATIO in range(0,RATIOPRECISION+1):
    ratio = RATIO/RATIOPRECISION
    D = min(max(1,int(TotalAgents*ratio)),TotalAgents-1)
    R = TotalAgents - D
    print(D,R)
