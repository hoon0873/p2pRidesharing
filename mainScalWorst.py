...
def main(n=5, DD = 2, PRECISION = 30, CAP = 4,rhoo=0):
    ...
    reqs = []
    drivers = []
    T = Distance((0,0), (1*PRECISION, 1*PRECISION))+PRECISION
...
##    timeF.seek(0)    
    ...
    print("#DRIVERS",D)	
    for i in range(D):
        ori = (int(0*PRECISION),int(0*PRECISION))

        des = (int(1*PRECISION),int(1*PRECISION))
...
timeF = open("ScalabilityWorst.txt", 'a+')
timeF.seek(0)
DD = 3
newRho = 1.2
