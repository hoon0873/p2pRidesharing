...
##    for i in range(D):
##        y.append([])
##        y[i] = m.addVars(G.edges(),vtype=grb.GRB.BINARY, lb=0, ub=1)
...
##            m.addConstr(y[i][e] == 0)
    ...
    for i in range(D):
        ...
        for t in range(drivers[i].et,drivers[i].lt-DIST(drivers[i])+1):
            ...
            BDO[i].add(EDGE)
            EDD[i].add(EDGE)
            y[i][EDGE] = m.addVar(vtype=grb.GRB.BINARY, lb=0,ub=1)
##            feaEdge[i].add(EDGE)
...
##                    feaEdge[i].add(((j,0,t+Distance(retLocation(dIndex,0),reqs[j].ori)),(j,0,reqs[j].et)))
                    y[i][((j,0,t+Distance(retLocation(dIndex,0),reqs[j].ori)),(j,0,reqs[j].et))] = m.addVar(vtype=grb.GRB.BINARY, lb=0,ub=1)
                    if t == drivers[i].et:
                        feasibleMat[i][j] = 1
    ...
    for j in range(R):
        for t in range(reqs[j].et,reqs[j].lt-DIST(reqs[j])+1):
            ...
            if t != reqs[j].lt-DIST(reqs[j]):
                ...
                for i in range(D):
                    if feasibleMat[i][j] == 1:
                        y[i][((j,0,t),(j,0,t+1))] =  m.addVar(vtype=grb.GRB.BINARY, lb=0,ub=1)
                        y[i][((j,1,t+DIST(reqs[j])),(j,1,t+DIST(reqs[j])+1))] =  m.addVar(vtype=grb.GRB.BINARY, lb=0,ub=1)
                ...
                if (retLocation(j,0) == retLocation(k,0)).all() and j>k:
                    ...
                    for i in range(D):
                        ...
                        if TIME <= drivers[i].lt:
                            y[i][EDGE] = m.addVar(vtype=grb.GRB.BINARY, lb=0,ub=1)
                            y[i][(k,0,t+Distance(retLocation(j,0),reqs[k].ori)),(k,0,reqs[k].et)] = m.addVar(vtype=grb.GRB.BINARY, lb=0,ub=1)
            ...
            for t in range(reqs[j].et,reqs[j].lt-DIST(reqs[j])+1):
                ...
                if (retLocation(j,1) == retLocation(k,0)).all() and j>k:
                    ...
                    for i in range(D):
                        ...
                        if TIME <= drivers[i].lt:
                            y[i][EDGE] = m.addVar(vtype=grb.GRB.BINARY, lb=0,ub=1)
                            y[i][(k,0,tt+Distance(retLocation(j,1),reqs[k].ori)),(k,0,reqs[k].et)] =  m.addVar(vtype=grb.GRB.BINARY, lb=0,ub=1)
##                            infeaEdge[i].add(EDGE)
...
### ADD CONSTRAINTS ###########################
    ...
    def OrinTim(t,i):   # return set of request origin edge start less than time t
        for e in RO.intersection(y[i]):
            if e[0][2] <= t: yield e
    def DesTim(t,i):    # return set of request destination edge less than time t
        for e in RD.intersection(y[i]):
            if e[0][2] <= t: yield e
    def OriRTim(t,i,j):   # return set of request j's origin edge less than time t
        for e in BRO[j].intersection(y[i]):
            if e[0][2] <= t: yield e
    def DesRTim(t,i,j):   # return set of request j's destin edge less than time t
        for e in BRD[j].intersection(y[i]):
            if e[0][2] <= t: yield e
    ...
    for i in range(D):
        ...
        for v in G.nodes():
            ...
            m.addConstr(grb.quicksum(y[i][e] for e in set(G.in_edges(v)).intersection(y[i]))
                        ==
                        grb.quicksum(y[i][e] for e in set(G.out_edges(v)).intersection(y[i]))
                        )
        # Driver need to be satisfied
        m.addConstr(grb.quicksum(y[i][e] for e in BDO[i])
                    == 1)
        m.addConstr(grb.quicksum(y[i][e] for e in EDD[i])
                    == 1)

    for j in range(R):
        m.addConstr(z[j]+grb.quicksum(y[i][e] for i in range(D) for e in BRO[j].intersection(y[i])) == 1)
        for i in range(D):
            m.addConstr(grb.quicksum(y[i][e] for e in BRO[j].intersection(y[i])) == grb.quicksum(y[i][e] for e in BRD[j].intersection(y[i])))
    ...
    for t in range(T):
        # CAPACITY CONSTRAINT
        for i in range(D):
            m.addConstr(grb.quicksum(y[i][e] for e in OrinTim(t,i)) - grb.quicksum(y[i][e] for e in DesTim(t,i))
                        <= drivers[i].cap)
        # ORIGIN BEFORE DESTINATION
            for j in range(R):
                m.addConstr(grb.quicksum(y[i][e] for e in OriRTim(t,i,j)) - grb.quicksum(y[i][e] for e in DesRTim(t,i,j))
                            >= 0)
    ...
    m.setObjective(grb.quicksum(c[e]*y[i][e] for i in range(D) for e in y[i]) + LAMBDA*grb.quicksum(z[j] for j in range(R)),
                   grb.GRB.MINIMIZE)
